...
in
{
  ...
  config = lib.mkIf cfg.enable {
    assertions = [
      ...
      {
        assertion = cfg.settings != null -> lib.foldl (a: b: a && b) true (lib.mapAttrsToList (mcu: _: mcu != null -> ((lib.hasAttrByPath [ "${mcu}" "serial" ] cfg.settings) || (lib.hasAttrByPath [ "mcu ${mcu}" "serial" ] cfg.settings))) cfg.firmwares);
